...
### 案1. フルホスト型（ユーザーのインストール: hook 設定のみ）

- HTML: Netlify / Vercel / GitHub Pages にデプロイ
- サーバー: Fly.io / Railway にデプロイ
- ユーザーがやること:
  1. ページを開く → ルーム ID が自動発行される
  2. ページに表示される hook 設定をコピーして `.claude/settings.json` に貼る
  3. 終わり
- メリット: ユーザー側のインストールがゼロ
- デメリット: サーバーの運用が必要、レイテンシ、ユーザー分離のためのルーム ID 管理

### 案2. npx 一発（Node.js があれば即使える）

- `npx agent-b2b` で起動 → サーバーが立ち上がって HTML も配信、ブラウザが自動で開く
- 起動時に hook 設定をターミナルに出力してコピペしてもらう
- ユーザーがやること:
  1. `npx agent-b2b`
  2. 表示された hook 設定をコピペ
  3. 終わり
- メリット: ローカル完結、レイテンシが小さい、サーバー運用不要
- デメリット: Node.js が必要

### 案3. Homebrew / シェルスクリプト

- `brew install agent-b2b` や `curl ... | sh` でバイナリをインストール
- Node.js 不要にするなら Go や Rust で書き直す必要がある
- メリット: ランタイム不要
- デメリット: 実装コストが高い

### 方針

localhost 完結とする。フルホスト型（案1）は hook から外部サーバーに POST することになり、Claude Code の入出力タイミングが第三者に漏れるセキュリティリスクがある。
リポジトリを clone してローカルで `node server.js` を実行する形で配布する。
